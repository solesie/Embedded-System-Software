...
#define ANDROMAN_HEAD 0
#define ANDROMAN_LEFT_EYE 1
#define ANDROMAN_RIGHT_EYE 2
#define ANDROMAN_LEFT_ANTENNA 3
#define ANDROMAN_RIGHT_ANTENNA 4
#define ANDROMAN_BODY 5
#define ANDROMAN_LEFT_HAND 6
#define ANDROMAN_RIGHT_HAND 7
#define ANDROMAN_LEFT_FOOT 8
#define ANDROMAN_RIGHT_FOOT 9
...
static GLuint androman_vb;

void androman_prepare(void) {
	...
	glGenBuffers(1, &androman_vb);
	glBindBuffer(GL_ARRAY_BUFFER, androman_vb);
...
void androman_draw(void){
	glBindBuffer(GL_ARRAY_BUFFER, androman_vb);
	glEnableVertexAttribArray(g2_loc_position);
	glVertexAttribPointer(g2_loc_position, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));

	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_HEAD]);
	glDrawArrays(GL_TRIANGLE_FAN, 0, 180);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_LEFT_EYE]);
	glDrawArrays(GL_TRIANGLE_FAN, 180, 360);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_RIGHT_EYE]);
	glDrawArrays(GL_TRIANGLE_FAN, 540, 360);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_LEFT_ANTENNA]);
	glDrawArrays(GL_TRIANGLE_FAN, 900, 4);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_RIGHT_ANTENNA]);
	glDrawArrays(GL_TRIANGLE_FAN, 904, 4);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_BODY]);
	glDrawArrays(GL_TRIANGLE_FAN, 908, 4);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_LEFT_HAND]);
	glDrawArrays(GL_TRIANGLE_FAN, 912, 4);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_RIGHT_HAND]);
	glDrawArrays(GL_TRIANGLE_FAN, 916, 4);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_LEFT_FOOT]);
	glDrawArrays(GL_TRIANGLE_FAN, 920, 4);
	glUniform3fv(g2_loc_primitive_color, 1, COLORS[ANDROMAN_RIGHT_FOOT]);
	glDrawArrays(GL_TRIANGLE_FAN, 924, 4);

	glDisableVertexAttribArray(g2_loc_position);
}

void androman_release(void){
	glDeleteBuffers(1, &androman_vb);
}